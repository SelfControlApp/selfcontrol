...
- (void)installDaemon:(void(^)(NSError*))callback {
    ...
    AuthorizationItem startBlockRight = {
        "com.application.SelfControl.corebits.startBlock", 0, NULL, 0
    };
    ...
    // Attempt removal, but log detailed error info if it fails.
    SILENCE_OSX10_10_DEPRECATION(
    SMJobRemove(kSMDomainSystemLaunchd, CFSTR("org.eyebeam.selfcontrold"), self->_authRef, YES, &cfError);
    );
    if (cfError) {
        NSError *removeErr = CFBridgingRelease(cfError);
        NSLog(@"WARNING: Failed to remove existing selfcontrold daemon. Domain=%@ Code=%ld UserInfo=%@",
              removeErr.domain, (long)removeErr.code, removeErr.userInfo);
        // reset for next use
        cfError = NULL;
    }
    ...
    if(!result) {
        ...
        // Log the full error so we can diagnose configuration/signing issues.
        NSLog(@"WARNING: Authorized installation of selfcontrold returned failure. Domain=%@ Code=%ld UserInfo=%@",
